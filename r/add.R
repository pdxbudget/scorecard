...
add_measures <- function(bureau, rows = NA) {
  ...
  df <- df %>%
    left_join(pos, "PO ID") %>%
    select(-`PO ID`) %>%
    rename(`Container ID` = id)  %>%
    mutate(`Container ID` = ifelse(
      `Container Type` == "Bureau",
      bureau_container_id, `Container ID`))
  
  if (anyNA(test$`Container ID`)) {
  ...
  for (i in 1:nrow(df)) {
    ...
    sc_add_measure(
      ...
      title = metric$`Measure Title`,
      container_id = po_container_id,
      calendar = "fy",
      data_source = metric$`SC-Data Source`,
      polarity = str_trunc(metric$`SC-Polarity`, 1, ellipsis = ""),
      scale = metric$`SC-Decimals`,
